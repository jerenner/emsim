...
class ROPEConfig:
    ...
    freq_group_pattern: Literal["single", "partition", "closure"] = "partition"
...
class TransformerConfig:
    ...
    n_feature_levels: int = "${array_length:model.modules.backbone.decoder.layers}"
    n_deformable_points: int = 4

    # Neighborhood Attention parameters
    neighborhood_sizes: Sequence[int] = field(default_factory=lambda: [3, 5, 7, 9])
    ...
    level_filter_ratio: Union[float, Sequence[float]] = field(
        default_factory=lambda: [0.25, 0.5, 1.0, 1.0]
    )
    layer_filter_ratio: Union[float, Sequence[float]] = field(
    ...
    mask_main_queries_from_denoising: bool = "${model.denoising.mask_main_queries_from_denoising}"
    ...
    def __post_init__(self):
        if isinstance(self.layer_filter_ratio, float):
            self.layer_filter_ratio = [self.layer_filter_ratio] * self.encoder.layers
        if len(self.layer_filter_ratio) != self.encoder.layers:
            raise ValueError(
