...
# Tests for init_2d_freqs
@pytest.mark.cuda
def test_init_2d_freqs_shape(device, dtype):
    head_dim = 64
    num_heads = 8
...
@pytest.mark.cuda
def test_init_2d_freqs_rotate_param(device, dtype):
    head_dim = 64
    num_heads = 8
...
# Tests for init_nd_freqs
@pytest.mark.cuda
def test_init_nd_freqs_shape(device, dtype):
    position_dim = 3
    head_dim = 64
    num_heads = 8
...
@pytest.mark.cuda
def test_init_nd_freqs_multiple_thetas(device, dtype):
    position_dim = 3
    head_dim = 64
    num_heads = 8
...
# Tests for RoPEEncodingND
def test_rope_encoding_nd_init(dtype):
    position_dim = 2
    d_model = 256
    n_heads = 8
    ...
    assert rope.head_dim == d_model // n_heads
    assert rope.freqs.shape == (n_heads, rope.head_dim // 2, position_dim)


@pytest.mark.cuda
def test_rope_encoding_nd_forward(device, dtype):
    position_dim = 2
    d_model = 256
    n_heads = 8
    batch_size = 2
...
def test_rope_encoding_nd_validation_errors():
    position_dim = 2
    d_model = 256
    n_heads = 8
    ...
    # Test wrong d_model
    query_wrong_dim = torch.randn(2, 10, d_model + 32)
    query_pos = torch.randn(2, 10, position_dim) * 10

    with pytest.raises(
        ValueError, match="Expected query_or_key to have last dim equal to d_model"
    ):
        rope(query_wrong_dim, query_pos)

    # Test wrong position_dim
    query = torch.randn(2, 10, d_model)
    query_pos_wrong_dim = torch.randn(2, 10, position_dim + 1) * 10
    ...
    query_pos_wrong_shape = (
        torch.randn(2, 15, position_dim) * 10
...
def test_rope_encoding_nd_grouped_freqs_init():
    position_dim = 3
    d_model = 256
    n_heads = 8
    ...
    rope = RoPEEncodingNDGroupedFreqs(
        position_dim, d_model, n_heads, pos_dim_to_rope_group
    )
...
@pytest.mark.cuda
def test_rope_encoding_nd_grouped_freqs_forward(device, dtype):
    position_dim = 3
    d_model = 256
    n_heads = 8
    batch_size = 2
...
def test_prep_multilevel_positions():
    ...
    assert torch.isclose(positions[1, 1], torch.tensor(expected_i, dtype=torch.float))


def test_prep_multilevel_positions_batched_shapes():
    # Sample indices
    bijl_indices = torch.tensor(
        [
            [0, 10, 20, 0],  # batch 0, level 0
